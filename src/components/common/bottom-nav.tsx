...
export function BottomNav() {
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-r from-green-600 to-teal-600/90 backdrop-blur-sm" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <nav className="flex h-16 items-center justify-around px-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {item.isCreate ? (
                 <div className="absolute top-1 left-1/2 -translate-x-1/2 flex h-12 w-12 items-center justify-center rounded-full overflow-hidden">
                   <Image
                     src="/Images/create%20button.jpg"
                     alt="Create"
                     width={48}
                     height={48}
