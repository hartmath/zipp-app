...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabaseAdmin.from('store_products').select('*').order('created_at', { ascending: false });
...
export async function PATCH(req: Request) {
  ...
  const { error } = await supabaseAdmin.from('store_products').update(updates).eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
